...
import { mobileCapabilities, userMobileCapabilities } from "../db/schema";
...
export function registerResponsibilityAdminRoutes(router: Router) {
  router.get(
    ...
    withAdminTenantDb<AdminRequest>(async (_req, res, db) => {
      ...
      const directMap = new Map(
        directCounts.map((row) => [row.capabilityId, Number(row.count)]),
      );
      ...
      return res.json({
        ...
        responsibilities: responsibilities
          ...
          .map((responsibility) => ({
            ...
            directAssignments: directMap.get(responsibility.id) ?? 0,
            assignmentRules: ruleMap.get(responsibility.id) ?? 0,
          })),
      });
    }),
  );
  ...
  router.post(
    ...
    withAdminTenantDb<AdminRequest>(async (req, res, db) => {
      const normalizedKey = normalizeKey(req.body?.key || req.body?.title);
      ...
      await ensureResponsibilityActions(db, created);
  ...
  router.patch(
    ...
    withAdminTenantDb<AdminRequest>(async (req, res, db) => {
      ...
      const [before] = await db
        .select()
        .from(mobileCapabilities)
        .where(eq(mobileCapabilities.id, id))
        .limit(1);
      ...
      if ("config" in (req.body ?? {})) {
        ...
        normalizeResponsibilityConfig(config);
      ...
      if (updated.isActive) {
        await ensureResponsibilityActions(db, updated);
      }
      ...
      return res.json({
        ...
        responsibility: {
          ...updated,
          definition: normalizeResponsibilityConfig(updated.config),
        },
      });
    }),
  );
   ...
   *
   * Business records are intentionally preserved.
   *
   * We only:
   * - deactivate the Responsibility,
   * - remove runtime assignments,
   * - mark it deleted for admin-list filtering.
   *
   * dynamic_submissions and historical workflow/business records remain.
   */
  router.delete(
    "/responsibilities/:id",
    withAdminTenantDb<AdminRequest>(async (req, res, db) => {
      ...
      const beforeConfig = objectValue(before.config);

      const deletedConfig = {
        ...beforeConfig,

        __brixtaDeleted: true,

        __brixtaDeletedAt: now.toISOString(),
      };
      ...
      /*
       * Remove delivery/assignment configuration.
       *
       * These are runtime-control records, not employee business data.
       */
      await db
        .delete(capabilityAssignmentRules)
        .where(eq(capabilityAssignmentRules.capabilityId, id));

      await db
        .delete(userMobileCapabilities)
        .where(eq(userMobileCapabilities.capabilityId, id));
      ...
      return res.json({
        ...
        message:
          "Responsibility removed. Existing submitted records were preserved.",
      });
    }),
  );
  ...
  router.post(
    ...
    withAdminTenantDb<AdminRequest>(async (req, res, db) => {
      const capabilityId = Number(
        req.body?.responsibilityId ?? req.body?.capabilityId,
      );
      ...
      if (
        ...
        return res.status(400).json({
          success: false,
          error: "Valid responsibilityId, subjectType and effect are required.",
        });
      }
        ...
        .values({
          ...
          subjectValue:
            subjectType === "all"
              ...
              : String(
                  subjectType === "role"
                    ? (req.body?.roleId ?? req.body?.subjectValue ?? "")
                    : (req.body?.subjectValue ?? ""),
                ).trim() || null,
          effect,
          priority: Number(req.body?.priority) || 0,
          config: objectValue(req.body?.config),
