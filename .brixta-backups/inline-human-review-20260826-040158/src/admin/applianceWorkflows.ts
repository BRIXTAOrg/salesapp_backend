...
async function createPublishedVersion(
  ...
  for (
    ...
    if (stepType === "approval") {
      ...
      if (!roleIds.length) {
        throw new Error(
          `Step ${index + 1}: at least one approver role is required.`,
        );
      }
    ...
    const [step] = await db
      ...
      .values({
        workflowVersionId:
          version.id,
        stepKey:
          `step_${index + 1}_${normalizeKey(title) || "node"}`,
        title,
        stepType,
        actionDefinitionId,
        approvalPolicyId,
...
export function registerWorkflowAdminRoutes(
  router: Router,
) {
  router.get(
    "/workflows",
        ...
        if (
          ...
          return res
            ...
            .json({
              success: false,
              error:
                "Valid workflow ID and steps are required.",
            });
        }
        ...
        try {
          ...
          return res
            ...
            .json({
              ...
              error:
                error?.message ??
                "Unable to publish workflow version.",
            });
        }
      },
    ),
