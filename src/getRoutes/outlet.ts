...
export default function setupOutletRoutes(app: Express) {
  ...
  // GET ALL OUTLETS
  // =====================================================
  app.get("/api/salesApp/outlets", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      const {
        search,
        limit = '50',
  ...
  // GET SINGLE OUTLET
  // =====================================================
  app.get("/api/salesApp/outlets/:id", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      const { id } = req.params;
