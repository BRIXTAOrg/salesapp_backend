...
export default function setupInstitutionRoutes(app: Express) {   
  ...
  app.get("/api/salesApp/institutions", authenticateToken, withTenantDb<AuthRequest>(async (req, res, db) => {     
    try {       
      ...
      if (sortBy === 'name') {         
        orderExpr = direction === 'asc' ? asc(institutions.institutionName) : desc(institutions.institutionName);       
  ...
  app.get("/api/salesApp/institutions/:id", authenticateToken, withTenantDb<AuthRequest>(async (req, res, db) => {     
    try {       
      const { id } = req.params;       
