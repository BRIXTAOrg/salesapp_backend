...
export async function requireAdminService(
  ...
  if (!expected) {
    console.error(
      "ADMIN_SERVICE_SECRET/FLOW1_ADMIN_SECRET is not configured.",
    );
    ...
    return res
      ...
      .json({
        success: false,
        error:
          "Admin service authentication is not configured.",
      });
  }

  const raw =
    req.headers[
      "x-admin-service-secret"
    ] ??
    req.headers[
    ...
    if (!org) {
      return res
        ...
        .json({
          success: false,
          error:
            "Unknown or unprovisioned tenant schema.",
        });
    }
...
export function withAdminTenantDb<
  Req extends AdminRequest =
    AdminRequest,
>(
  handler: (
    req: Req,
  ...
  return async (
    ...
    try {
      ...
      console.error(
        "Tenant-scoped admin route error:",
        error,
      );
