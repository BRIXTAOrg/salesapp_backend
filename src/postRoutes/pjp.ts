...
export default function setupPjpPostRoutes(app: Express) {
  app.post("/api/salesApp/permanent-journey-plans/bulk", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      ...
      const formattedPlans = plans.map((plan: any) => {
        // 1. Destructure to strip out invalid IDs and null timestamps from Flutter
        ...
        return {
          ...
          // 2. Explicitly generate timestamps on the server to prevent Postgres null errors
          createdAt: new Date(),
          updatedAt: new Date(),

          // Ensure the new integer fields are correctly parsed if sent as strings by accident
